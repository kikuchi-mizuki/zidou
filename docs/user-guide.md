...
**次にすること**:
- 発注件数が表示されている場合 → 「自動計算を実行」ボタンをクリック
- 在庫アラートが表示されている場合 → 「在庫管理」で在庫を追加
...
画面上部に商品別の在庫合計が表示されます：
- **数字が緑色**: 在庫に余裕あり
- **「在庫少」バッジ**: 10ケース未満（発注を検討）
...
**A**: ページを再読み込み（F5キー）してください。それでも表示されない場合は、もう一度登録してください。
...
**A**:
1. すべての在庫の賞味期限が短すぎる可能性があります
2. 以下のいずれかを実施：
   - 賞味期限が長い在庫を追加
   - 希望納品日を早める
   - ルール管理で「納品時30日以上」を一時的に無効化（非推奨）
...
- ページを再読み込みすると初期データが表示されます
- 追加したデータは再度登録が必要です
- **本番版ではデータベースに保存されるため、この問題は発生しません**
...
1. **実際のデータで試す**: まずは少ない件数で操作を確認
2. **業務フローを確認**: 実際の業務に合わせて使い方を調整
3. **本番環境へ**: データベース連携など本格的な運用を検討
